...
class _BaseAttentionMechanism(AttentionMechanism):
    ...
    def __init__(self,
        ...
        self._probability_fn = lambda score, prev: (probability_fn(_maybe_mask_score(score, memory_sequence_length, score_mask_value), prev))
        with ops.name_scope(
...
def _bahdanau_score(processed_query, keys, normalize):
    ...
    print('processed_query expand', processed_query)
    ...
    if normalize:
        ...
        print('before add', keys, processed_query)
        print('add', keys + processed_query)
        print('tanh add', math_ops.tanh(keys + processed_query))
        print('tanh v add', v * math_ops.tanh(keys + processed_query))
        print('sum tanh v add', math_ops.reduce_sum(v * math_ops.tanh(keys + processed_query), [2]))
...
class BahdanauAttention(_BaseAttentionMechanism):
    ...
    def __call__(self, query, previous_alignments):
        ...
        with variable_scope.variable_scope(None, "bahdanau_attention", [query]):
            print('query', query)
            print('_keys', self._keys)
            print('_normalize', self._normalize)
            processed_query = self.query_layer(query) if self.query_layer else query
            print('processed_query', processed_query)
            # processed_query: s_{i-1}  [batch_size, hidden_units]
            # keys: hj [batch_size, encode_max_step, hidden_units]
            score = _bahdanau_score(processed_query, self._keys, self._normalize)
            print('score', score)
            print('FN', score, previous_alignments)
        alignments = self._probability_fn(score, previous_alignments)
...
class BahdanauMonotonicAttention(_BaseMonotonicAttentionMechanism):
    ...
    def __call__(self, query, previous_alignments):
        ...
        with variable_scope.variable_scope(
            ...
            print('processed_query', query)
            processed_query = self.query_layer(query) if self.query_layer else query
            print('_keys', self._keys, '_normalize', self._normalize)
            score = _bahdanau_score(processed_query, self._keys, self._normalize)
            print('score', score)
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
    ...
    def call(self, inputs, state):
        ...
        print('Cell inputs', cell_inputs)
        cell_state = state.cell_state
        
        print('cell_state', cell_state)
        
        cell_output, next_cell_state = self._cell(cell_inputs, cell_state)
        print('cell_output', cell_output, 'next_cell_state', next_cell_state)
        cell_batch_size = (
            cell_output.shape[0].value or array_ops.shape(cell_output)[0])
        
        print('cell_batch_size', cell_batch_size)
        error_message = (
        ...
        print('previous_alignments', previous_alignments)
        print('previous_alignment_history', previous_alignment_history)
        
        all_alignments = []
        all_attentions = []
        all_histories = []
        print('_attention_layers', self._attention_layers)
